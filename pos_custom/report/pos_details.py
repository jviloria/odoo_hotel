...
class pos_details(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self, form):
        _logger.critical("MONOCUCO")
        _logger.critical("AGRUPADO: %s"%form['grouped'])
        ...
        if not form['grouped']:
            ...
            data = []
            self.product_list = []
            orders = pos_obj.browse(self.cr, self.uid, pos_ids, context=self.localcontext)
            for order in orders:
                for line in order.lines:
                    if line.product_id.id not in self.product_list:
                        self.product_list.append(line.product_id.id)
            for code in self.product_list:
                result = self._locate_products(orders, code)
                if result:
                    data.append(result)
        if data:
            return data
    ...
    def _locate_products(self, orders, code):
        product_total_qty = 0
        product_total_price = 0
        pos_name = ''
        for order in orders:
            for line in order.lines:
                if line.product_id.id == code:
                    product_total_qty = product_total_qty + line.qty
                    product_total_price = product_total_price + (line.price_unit * line.qty)
                    pos_name = line.product_id.name
        result = {
            'code': code,
            'pos_name': pos_name,
